...
async def lifespan(_: FastAPI):
    ...
    logger.debug("Invalidating settings_cache")
    await get_settings_cache().invalidate()
    logger.debug("Invalidating settings_cache DONE")
    logger.debug("Invalidating rate_limit_headers_cach ")
    await get_rate_limit_headers_cache().invalidate()
    logger.debug("Invalidating rate_limit_headers_cach DONE")
    logger.debug("reload_additional_quota_registry")
    reload_additional_quota_registry()
    logger.debug("reload_additional_quota_registry DONE")
    logger.debug("init_db")
    await init_db()
    logger.debug("init_db DONE")
    logger.debug("init_http_client(")
    await init_http_client()
    logger.debug("init_http_client( DONE")
    logger.debug("build_usage_refresh_scheduler")
    usage_scheduler = build_usage_refresh_scheduler()
    logger.debug("build_usage_refresh_scheduler DONE")
    logger.debug("build_model_refresh_scheduler")
    model_scheduler = build_model_refresh_scheduler()
    logger.debug("build_model_refresh_scheduler DONE")
    logger.debug("build_sticky_session_cleanup_scheduler")
    sticky_session_cleanup_scheduler = build_sticky_session_cleanup_scheduler()
    logger.debug("build_sticky_session_cleanup_scheduler DONE")
    logger.debug("Starting usage_scheduler")
    ...
    await sticky_session_cleanup_scheduler.start()
    logger.debug("Starting cleanup_scheduler DONE")
...
def create_app() -> FastAPI:
    ...
    app.include_router(proxy_api.anthropic_v1_router)
